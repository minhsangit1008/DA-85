...
# Thiết lập logging
logging.basicConfig(level=logging.INFO, filename='result_multiprocessing.log', filemode='w',
...
def calculate_WIO_WIOUB(itemset, fp_tree, WTO, weight_dict):
...
def evaluate_item(item, prefix, fp_tree, WTO, weight_dict, MinWIO):
    new_prefix = prefix + [item]
    WIO, WIOUB, tids = calculate_WIO_WIOUB(new_prefix, fp_tree, WTO, weight_dict)
    return (item, new_prefix, WIO, WIOUB, tids)
...
def fp_growth(fp_tree, WTO, weight_dict, MinWIO, prefix=None, HOI=None):
    ...
    for item in fp_tree.header_table:
        wio_ub = estimate_WIOUB(item, fp_tree, WTO, weight_dict)
        if wio_ub >= MinWIO:
            items.append(item)
    items = sorted(items, key=lambda x: fp_tree.item_counts[x])
    ...
    with Pool(processes=cpu_count()) as pool:
        results = pool.starmap(
            evaluate_item,
            [(item, prefix, fp_tree, WTO, weight_dict, MinWIO) for item in items]
        )

    for item, new_prefix, WIO, WIOUB, tids in results:
        ...
        if WIOUB >= MinWIO:
            ...
            if cond_tree.header_table:
                fp_growth(cond_tree, WTO, weight_dict, MinWIO, new_prefix, HOI)
    return HOI
...
def tune_parameters(transactions_file, weights_file):
    ...
    min_wio_values = [0.01, 0.03, 0.05, 0.07, 0.1]
    min_ws_values = [0.01, 0.03, 0.05, 0.07, 0.1]
    results = []
    ...
    results_df.to_csv('result_multiprocessing.csv', index=False)
    logging.info("Results saved to result_multiprocessing.csv")
    print("\nResults saved to result_multiprocessing.csv")
